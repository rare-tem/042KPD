...
/* Exported types ------------------------------------------------------------*/
	...
	typedef struct
	{
		...
		uint32_t PeriphOrM2MSrcIncMode; /*!< Specifies whether the Peripheral address or Source address in case of
										   memory to memory transfer direction is incremented or not. This parameter can
										   be a value of @ref DMA_LL_EC_PERIPH

											 This feature can be modified afterwards using unitary function @ref
										   LL_DMA_SetPeriphIncMode(). */

		uint32_t MemoryOrM2MDstIncMode; /*!< Specifies whether the Memory address or Destination address in case of
										   memory to memory transfer direction is incremented or not. This parameter can
										   be a value of @ref DMA_LL_EC_MEMORY
...
#define __LL_DMA_GET_CHANNEL_INSTANCE(__DMA_INSTANCE__, __CHANNEL__)                                                   \
	...
	{
		return (
			READ_BIT(((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)DMAx + CHANNEL_OFFSET_TAB[Channel - 1U])))->CNDTR,
					 DMA_CNDTR_NDT));
	}
	...
	{
		return (
			READ_REG(((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)DMAx + CHANNEL_OFFSET_TAB[Channel - 1U])))->CMAR));
	}
	...
	{
		return (
			READ_REG(((DMA_Channel_TypeDef *)((uint32_t)((uint32_t)DMAx + CHANNEL_OFFSET_TAB[Channel - 1U])))->CPAR));
	}
	...
	{
		MODIFY_REG(DMAx->CSELR, DMA_CSELR_C1S << ((Channel - 1U) * 4U), PeriphRequest << DMA_POSITION_CSELR_CXS);
	}
	...
	{
		return (READ_BIT(DMAx->CSELR, DMA_CSELR_C1S << ((Channel - 1U) * 4U)) >> DMA_POSITION_CSELR_CXS);
	}
#endif
